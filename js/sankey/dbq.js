const nsDbq = {
	async Promises() {
	  ...
	  const dsPromises = datasetNames.map(async (datasetName) => {
		const queryFilter = { flow: "all" };
		const query = queryBuilder(datasetName, queryFilter);
		const data = await fetchDataset(query);
		return this.parsePromise(data);
	  });
  
	  return Promise.all(dsPromises);
	},
  
	parsePromise(data) {
	  const dimensions = ["geo", "nrg_bal", "siec", "time"];
	  const sankeyDBnew = {
		value: data.value,
		mask: data.value.map(() => true),
		dimension: {
		  id: ["geo", "flow", "fuel", "year"],
		  size: dimensions.map(dim => data.Dimension(dim).length),
		  ...dimensions.reduce((acc, dim) => {
			acc[dim === "nrg_bal" ? "flow" : dim] = {
			  index: data.Dimension(dim).id,
			  label: {},
			};
			return acc;
		  }, {}),
  ...
  function queryBuilder(datasetName, filters = {}) {
