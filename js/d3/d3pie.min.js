...
(this, function () {
	...
	h = {
		...
		computePieRadius : function (a) {
			...
			/%/.test(b.pieInnerRadius) ? (g = parseInt(b.pieInnerRadius.replace(/[\D]/, ""), 10), g = g > 99 ? 99 : g, g = 0 > g ? 0 : g, f = Math.floor(h / 100 * g)) : f = parseInt(b.pieInnerRadius, 10),
			a.innerRadius = f,
			a.outerRadius = h
		},
	...
	i = {
		add : function (a, b, c) {
			...
			d.mainLabel && g.append("text").attr("id", function (c, d) {
				...
				var c = a.label;
				return e.formatter ? (h.index = b, h.part = "mainLabel", h.value = a.value, h.label = c, c = e.formatter(h)) : e.truncation.enabled && a.label.length > e.truncation.truncateLength && (c = a.label.substring(0, e.truncation.truncateLength) + "..."),
				c
			...
			d.percentage && g.append("text").attr("id", function (c, d) {
				...
				return e.formatter ? (h.index = c, h.part = "percentage", h.value = b.value, h.label = d, d = e.formatter(h)) : d += "%",
				d
	...
	l = {
		...
		//Initial function generating IE bug (tooltips not hiding when mouseout)
		...
		showTooltip: function(pie, index) {
			...
			if (l.currentTooltip === index) {
				fadeInSpeed = 1;
			}
			//IE11 Tooltip hangup fix https://github.com/benkeen/d3pie/issues/43
			if (l.currentTooltip !== index) {
				this.hideTooltip(pie, l.currentTooltip);
			}
			l.currentTooltip = index;
		...
		moveTooltip : function (a) {
			d3.selectAll("#" + a.cssPrefix + "tooltip" + l.currentTooltip).attr("transform", function () {
				var d = f.getDimensions(a.cssPrefix + "tooltip" + l.currentTooltip);
				var tooltipWidth = d.w + 2 * a.options.tooltips.styles.padding;
				var b = d3.mouse(this.parentNode),
				c = b[0] + a.options.tooltips.styles.padding + 2,
				d = b[1] - 2 * a.options.tooltips.styles.padding - 2;
				var x1Tooltip = c;
				var x2Tooltip = c + tooltipWidth;
				if(x2Tooltip > a.options.size.canvasWidth){
					c = c - (x2Tooltip - a.options.size.canvasWidth);
				}
	...
	var n = function () {
		...
		f.whenElementsExist(b, function () {
			...
			if (a.textComponents.title.exists || a.textComponents.subtitle.exists) {
				var d = 0;
				a.textComponents.title.exists && (d += a.textComponents.title.h, a.textComponents.subtitle.exists && (d += a.options.header.titleSubtitlePadding)),
